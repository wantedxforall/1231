...
@endif
@if(isset($destroy))
<a title="Delete" onclick="if(confirm('Are You Sure ?')){document.getElementById('form-{{ $id }}').submit()}" class="btn btn-danger actions"><i class="bi bi-trash3"></i></a>
<form id="form-{{ $id }}" action="{{ $destroy }}" method="POST">
    @csrf
    @method('DELETE')
</form>
@endif
@if(isset($show))
<a href="{{ $show }}" title="Show" class="btn btn-success actions"><i class="bi bi-arrow-right"></i></a>
@endif
...
@if(isset($login))
<a title="Login" onclick="document.getElementById('form-login-{{ $id }}').submit()" class="btn btn-success actions" style="margin-right: 10px"><i class="fa fa-user"></i></a>
<form id="form-login-{{ $id }}" action="{{ $login }}" method="POST">
    @csrf
</form>
@endif
